...
export function ProjectsSection() {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: globalThis.WheelEvent) => {
      ...
      if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
        ...
        const isAtEnd = activeIndex === numProjects - 1;

        // Momentum Lock: If we hit the boundary, absorb all remaining inertial scrolling.
        // We ONLY release the page scroll if the user deliberately paused (300ms) and scrolled again.
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Shortened auto progression to 2 seconds as requested
